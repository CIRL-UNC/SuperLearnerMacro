...
- **library**: [value =  a space separated list of learners] the names of the *m* level-0 models (e.g. glm lasso cart). A single learner can be used here if you only wish to know the cross-validated expected loss (e.g. mean-squared error). See [all available default learners here](docs/availablelearners.md "Available learners") and [how to construct new learners here](docs/newlearners.md "Custom learners")
